 ...
 class myRSSParser
 {
  ...
  function myRSSParser($file)
  {
	  ...
	  xml_set_character_data_handler($xml_parser, "parseData");
	   ...
	   {         
		xml_parse($xml_parser, $data, feof($fp)) or die(sprintf("myRSSParser: Error <b>%s</b> at line <b>%d</b><br>",
   ...
   if($tagname == "ENCLOSURE" && $attrs) {
	...
	$this->endElement($parser, "ENCLOSURE");
	 ...
	 if(preg_match("/^(RDF|RSS|CHANNEL|IMAGE|ITEM|ENTRY|FEED)/", $tagname)) {
	 if($this->tags) {
			...
			if(is_array($tag)) {
				...
				else 
				{
					/*if(preg_match("/^([A-Z]+):([A-Z]+)$/", $tag, $matches)) {
						$evalcode .= "[\"".strtolower($matches[1])."\"][\"".strtolower($matches[2])."\"]";
					} 
					else 
					{*/
						$evalcode .= "[\"".strtolower($tag)."\"]";
					/*}*/
				}
		 } 
	 ...
	 function display_channel($data, $limit)
	 {
		...
		{ 
			extract($data);       if(!$TITLE) return;$this->retval .=  "<p><b>";	
			...
			elseif($DESCRIPTION) {
				if($IMAGE) { foreach($IMAGE as $IMG) $this->retval .= "<img src=\"$IMG\">\n";	
			}
		...
		{   
			...
			switch($encoding)
			{
				case 'ASCII':                                                                                                                                                
				case 
					$output_encoding: break;            
				case '': 
					$input = mb_convert_encoding($input, $output_encoding); break; 
					
				default: 
					$input = mb_convert_encoding($input, $output_encoding, $encoding); break;                                                                                                                                              
			}     
		} 
