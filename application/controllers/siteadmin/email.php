...
class Email extends CI_Controller
{
	...
	function index()
	{		
		$data["open_tab"]="email_box";
		
		//Get All Email Termplates List
		$data["title"]=translate_admin('Manage Email Template');
		
		$data['email_settings']	=	$this->Email_model->getEmailSettings();

		$data['contents'] = $this->load->view("admin/email/template",array(),TRUE);
		
		$this->load->view('admin/email_file', $data);	
  ...
  /**
	...
	function addemailTemplate()
	{
		...
		//Load View
			$data["open_tab"]="email_box";
			$data["title"]=translate_admin("Add E-mail Template");
			
			$data['contents'] = $this->load->view("admin/email/add_template",array(),TRUE);
