 ...
 */
	 ...
	 */
	 function updateSiteSettings($updateData=array())
	 {
		...
		if(isset($updateData['site_logo_image']))
		{
			$data = array('text_value' =>$updateData['site_logo_image']);
			$this->db->where('code', 'SITE_LOGO');
			$this->db->update('settings', $data);
		}
		
		if(isset($updateData['google_adsense_id']))
		{
			$data = array('text_value' =>$updateData['google_adsense_id']);
			$this->db->where('code', 'GOOGLE_AD_SENSE');
			$this->db->update('settings', $data);
		}
		
		if(isset($updateData['site_meta_tag']))
		{
			$data = array('text_value' =>$updateData['site_meta_tag']);
			$this->db->where('code', 'META_TAGS');
			$this->db->update('settings', $data);
		}
		
		if(isset($updateData['site_meta_description']))
		{
			$data = array('text_value' =>$updateData['site_meta_description']);
			$this->db->where('code', 'META_TAGS_DESCRIPTION');
			$this->db->update('settings', $data);
		}
		if(isset($updateData['site_time_zone']))
		{
			$data = array('text_value' =>$updateData['site_time_zone']);
			$this->db->where('code', 'SITE_TIME_ZONE');
			$this->db->update('settings', $data);
		}
